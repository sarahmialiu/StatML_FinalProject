...
class VisiumPoEDataSet(VisiumDataset):
    ...
    def __init__(
        ...
        self.features = adata.uns['morph'].astype(np.float64)
        self.map_info = args.map_info
        self.r = args.params['patch_r']
        self.spot_feature_stack = [x for x in self.features]

        assert self.features is not None,\
            "Empty features input," \
    ...
    def __getitem__(self, idx):
        ...
        spot_feature_stack = self.spot_feature_stack[idx]
        return (sample,

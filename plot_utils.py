...
def pl_spatial_inf_morph_feature(
    adata,
    feature,
    morph_column=None,
    vmin=0,
    ...
    if feature == 'qz_m':
        # Visualize deconvolution on UMAP of inferred Z-space
        qz_u = get_z_umap(adata_pl.obs.values)
        morph_df = adata.uns['morph']
        if isinstance(morph_column, list):
            for column in morph_column:
                title = 'Column ' + str(column) + ': (Inferred proportion - UMAP of Z)'
                pl_umap_feature(qz_u, morph_df[:, column], cmap, title,
                                vmin=vmin, vmax=vmax)
        else:
            title = 'Column ' + str(morph_column) + ': (Inferred proportion - UMAP of Z)'
            fig, ax = pl_umap_feature(qz_u, morph_df[:, morph_column], cmap, title,
                            vmin=vmin, vmax=vmax)
            return fig, ax
    else:
        raise ValueError('Invalid Starfysh inference results `{}`, please choose from `qc_m`, `qz_m` & `ql_m`'.format(feature))

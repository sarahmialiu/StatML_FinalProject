...
class AVAE_PoE(nn.Module):
	...
	features_col: str
		Morphological features (avg and std) captured for each patch 
        ...
        self.img_z_enc = nn.Sequential( 
            nn.Linear(self.num_features, self.c_hidden, bias=True), 
                # this network is trying to multiply matrices: 32x38 and 39x64
            nn.BatchNorm1d(self.c_hidden, momentum=0.01, eps=0.001),
            nn.ReLU()
        )
        ...
        self.py_mu_decoder = nn.Sequential(
            nn.Linear(self.c_hidden, self.num_features),
            nn.BatchNorm1d(self.num_features, momentum=0.01, eps=0.001),
            nn.ReLU()
        )
    ...
    def predictor_img(self, y):
        ...
        # q(z | y)
        hidden_z = self.img_z_enc(y_n) # matrix multiplication error here
    ...
    def get_loss(
        ...
        reconst_loss_x_poe = -NegBinom(px_rate_poe, torch.exp(px_r_poe)).log_prob(x).sum(-1).mean() 
            # Should we be getting loss from negative binomial distribution or normal distribution?
...
def train_poe(
    ...
    for i, (x,
        ...
        generative_outputs = model.generative(inference_outputs, xs_k)
        img_outputs = model.predictor_img(img.float()) # inference & generative for 2D img. data
...
def model_eval(
    ...
    with torch.no_grad():
        ...
        if poe:
            ...
            img_in = torch.Tensor(adata.uns['morph'].astype(np.float64)).float().to(device)
